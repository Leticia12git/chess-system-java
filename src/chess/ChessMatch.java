...
public class ChessMatch {
	...
	public ChessPiece performChessMoves(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if (testCheck(currentPlayer)) {
			...
			throw new ChessPosition("You can't put yourself in check");
		}
	...
	private ChessPiece king(Color color) {
		List<Piece> list = piecesOnTheBoard.stream().filter(x -> (ChessPiece)x).getColor()==color).colllect(Collectors.toList());
		for(Piece p : list) {
	...
	private testCheck(Color color) {
		...
		private boolean testCheckMate(Color color) {
			...
			List<Piece> piecesOnTheBoard.stream().filter(x -> (ChessPiece)x).getColor() == opponent(color).collect)Collectors.toList());
			for(Piece p : list) {
				boolean[][] mat = p.possibleMoves();
